...
MIN_XP_BY_LEVEL = {
    1: 150, 2: 300, 3: 450, 4: 475, 5: 750, 6: 900, 7: 1100, 8: 1400,
    9: 1600, 10: 1750, 11: 1875, 12: 2000, 13: 2000, 14: 2500, 15: 2500,
    16: 2500, 17: 2500, 18: 2500, 19: 2500, 20: 2500
}
...
async def on_ready():
    ...
    print(f'✅ XP tracking is enabled for channels: {ALLOWED_CHANNEL_IDS} and their threads.')
...
async def xptrack(ctx, level: int = None):
    ...
    if level is None:
        if user_id in xp_tracking:
            await ctx.send(f"{ctx.author.mention}, XP tracking is already active!")
            return

        xp_tracking[user_id] = {"start_time": time.time(), "word_count": 0, "message_count": 0}
        await ctx.send(f"{ctx.author.mention}, XP tracking started! Your roleplay will now count towards XP.")

    else:
        if user_id not in xp_tracking:
            await ctx.send(f"{ctx.author.mention}, you haven't started XP tracking yet!")
            return

        if level not in MIN_XP_BY_LEVEL:
            await ctx.send(f"{ctx.author.mention}, please enter a valid level between 1 and 20.")
            return
        ...
        await ctx.send(f"{ctx.author.mention}, XP tracking ended! You earned **{earned_xp} XP**.")

# ====== TRACK MESSAGES ======
@bot.event
async def on_message(message):
    """Tracks words while XP tracking is active."""
    if message.author.bot:
        return

    if message.channel.id in ALLOWED_CHANNEL_IDS or (
            isinstance(message.channel, discord.Thread) and message.channel.parent_id in ALLOWED_CHANNEL_IDS):
